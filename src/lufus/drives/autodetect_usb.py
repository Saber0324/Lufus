...
class UsbMonitor(QObject):
    ...
    def _load_existing(self):
        ...
        for device in self.context.list_devices(subsystem='block', DEVTYPE='disk'):
            if device.get('ID_BUS') == 'usb':
                ...
                print(f"UsbMonitor: found existing USB device: {node} label={label!r} "
    ...
    def _event(self, device):
        ...
        print(f"UsbMonitor: udev event -> action={action}, node={node}, label={label!r}, "
        ...
        if action == "add":
            ...
            if node in self.devices:
                ...
                print(f"UsbMonitor: device removed: {node} (was labeled {removed_label!r})")
